    ...
    def control(self, context: PygameContext):
        if self.live:
            if self.state == FirstBoss.STATE_INIT:
                self.y = self.y + 2
                if self.y >= 0:
                    self.run_dest = [((context.width - self.width), (context.height - self.height)), ((context.width - self.width), self.height)]
                    self.state = FirstBoss.STATE_RUNS
                    self.dir_x, self.dir_y = get_direction(self.x, self.y, (self.run_dest[0])[0], (self.run_dest[0])[1])
            ...
            if self.state == FirstBoss.STATE_RUNS:
                ...
                if self.x <= (self.run_dest[self.run_number])[0]:
                    self.run(d_t)
                else:
                    if self.run_number == 0:
                        self.dir_x, self.dir_y = get_direction(self.x, self.y, (self.run_dest[1])[0], (self.run_dest[1])[1])
                        self.x_0, self.y_0 = self.x, self.y
                        self.run_number = self.run_number + 1
                        self.start_time = context.time

                if self.run_number == 1 and self.y >= 0:
                    self.run(d_t)
